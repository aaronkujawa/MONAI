...
class TestScaleIntensityFixedMean(NumpyImageTestCase2D):
    ...
    def test_preserve_range(self, p):
        for channel_wise in [False, True]:
            factor = 0.9
            scaler = ScaleIntensityFixedMean(factor=factor, preserve_range=True, channel_wise=channel_wise, fixed_mean=False)
            im = p(self.imt)
            result = scaler(im)
    ...
    def test_fixed_mean_preserve_range(self, p):
        for channel_wise in [False, True]:
            factor = 0.9
            scaler = ScaleIntensityFixedMean(factor=factor, preserve_range=True, fixed_mean=True, channel_wise=channel_wise)
            im = p(self.imt)
            clip_min = im.min()
            clip_max = im.max()
            result = scaler(im)
