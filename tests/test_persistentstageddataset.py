...
from monai.data import PersistentStagedDataset, json_hashing
...
class TestDataset(unittest.TestCase):
    ...
    def test_different_transforms(self):
        ...
        with tempfile.TemporaryDirectory() as path:
            im1 = PersistentStagedDataset(data=[im], new_transform=Identity(), old_transform=None, cache_dir=path)[0]
            im2 = PersistentStagedDataset(data=[im], new_transform=Flip(1), old_transform=None, cache_dir=path)[0]
    ...
    def test_old_transform(self):
        """
        Different instances of `PersistentDataset` with the same cache_dir,
        same input data, but different transforms should give different results.
        """
        shape = (1, 2, 2, 2)
        im = torch.arange(0, np.prod(shape)).reshape(shape)
        flip_tfm = self.FlipCallCheck(1)
        with tempfile.TemporaryDirectory() as path:
            # step 1: cache im data transformed with new_transform (flip)
            PersistentStagedDataset(data=[im], new_transform=flip_tfm, old_transform=None, cache_dir=path)
            # step 2: apply new_transform (flip again) to cached data ([im]+old_transform points to cached data)
            im2 = PersistentStagedDataset(data=[im], new_transform=flip_tfm, old_transform=flip_tfm, cache_dir=path)[0]

            # check output: flipping twice should return the original data
            torch.testing.assert_close(im, im2)

            # make sure that the Flip transform was called only 2 times (once in step 1 and once in step 2, i.e., the
            # cached data was found and loaded in step 2, and the old_transform was not applied again.
            self.assertTrue(flip_tfm.call_counter == 2)
